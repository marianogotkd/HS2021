Public Class Mant_consulta
    ...
    Private Sub recuperar_mantenimientos()
        Dim ds_mant As DataSet = daMant.Mantenimiento_realizado_buscar(fecha.Value.Date, SucxClie_id)
        ...
        Else
            ...
            If ds_mant.Tables(0).Rows.Count <> 0 Then
                MANT_ds.Tables("mant_listados").Merge(ds_mant.Tables(1))
            End If
            ...
            If ds_mant.Tables(3).Rows.Count <> 0 Then
                MANT_ds.Tables("mant_realizados_detalle").Merge(ds_mant.Tables(3))
            End If
                        ...
                        If ds_mant.Tables(4).Rows.Count <> 0 Then
                            ...
                            While ii < ds_mant.Tables(4).Rows.Count
                                If Mantenimiento_id = ds_mant.Tables(4).Rows(ii).Item("Mantenimiento_id") Then
                                    ...
                                    Dim j As Integer = 0
                                    While j < MANT_ds.Tables("mant_realizados_detalle").Rows.Count
                                        If (Mantenimiento_id = MANT_ds.Tables("mant_realizados_detalle").Rows(j).Item("Mantenimiento_id")) And (Cat2_caract_id = MANT_ds.Tables("mant_realizados_detalle").Rows(j).Item("Cat2_caract_id")) Then
                                            valido = "no"
                                            Exit While
                                        End If
                                        j = j + 1
                                    ...
                                    If valido = "si" Then
                                        'lo agrego
                                        Dim fila As DataRow = MANT_ds.Tables("mant_realizados_detalle").NewRow
                                        ...
                                        fila("Mant_detalle") = ""
                                        fila("Estado") = "nuevo"
                                        MANT_ds.Tables("mant_realizados_detalle").Rows.Add(fila)
                                    End If
                                End If
                                ii = ii + 1
                            End While
    ...
    Private Sub validar_y_agregar(ByVal Mantenimiento_id As Integer, ByVal datos As DataTable, ByVal indice As Integer)
        ...
        Dim valido As String = "si"
        Dim i As Integer = 0
        While i < MANT_ds.Tables("mant_listados").Rows.Count
            If Mantenimiento_id = MANT_ds.Tables("mant_listados").Rows(i).Item("Mantenimiento_id") Then
                valido = "no"
                Exit While
            End If
            i = i + 1
        ...
        If valido = "si" Then
            'entonces agrego
            Dim fila As DataRow = MANT_ds.Tables("mant_listados").NewRow
            ...
            MANT_ds.Tables("mant_listados").Rows.Add(fila)
        End If
    ...
    Private Sub Mant_consulta_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        fecha.Value = "07/06/2021"
        recuperar_mantenimientos()

        'aplicar(filtro)
        If DG_clientes.Rows.Count <> 0 Then
            If MANT_ds.Tables("mant_realizados_detalle").Rows.Count <> 0 Then
                aplicarfiltro_mant_realizados_detalle(DG_clientes.Rows(0).Cells("Mantenimiento_id").Value,
    ...
    Private Sub DG_clientes_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles DG_clientes.Click
        Try
            If MANT_ds.Tables("mant_realizados_detalle").Rows.Count <> 0 Then
                aplicarfiltro_mant_realizados_detalle(CInt(DG_clientes.CurrentRow.Cells("Mantenimiento_id").Value),
