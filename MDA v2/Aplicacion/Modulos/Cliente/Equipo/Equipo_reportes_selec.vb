Public Class Equipo_reportes_selec
    ...
    End Sub
    Public ds_clie As DataSet
    Private Sub recuperar_info_cliente()
        ds_clie = dacliete.Cliente_obtener_info(Cli_id)
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        If rb_1_select.Checked = True Then
            ...
            End If
        Else
            'aqui ciclo hasta obtener todo.
            Dim i As Integer = 0
            While i < ds_clie.Tables(3).Rows.Count
                Dim sucxclie_id As Integer = ds_clie.Tables(3).Rows(i).Item("SucxClie_id")
                Dim sucursal_text As String = ds_clie.Tables(3).Rows(i).Item("SucxClie_nombre")
                reporte_B(sucxclie_id, sucursal_text)
                i = i + 1
            End While
        End If
    ...
    Private Sub reporte_B(ByVal SucxClie_id As Integer, ByVal sucursal_text As String) 'ESTE LO USO EN EL CICLO, POR LO TANTO DEBO PASAR COMO PARAMETRO EL ID DE LA SUC
        Try
            ...
            If rb_todos_suc.Checked = True Then
                'traer todos los equipos ordenados por sector de una sucursal.
                Dim ds_equipos As DataSet = DAequipo.Equipo_obtener_todos_de_sucursal(SucxClie_id)
                If ds_equipos.Tables(0).Rows.Count <> 0 Then
                    Dim ii As Integer = 0
                    While ii < ds_equipos.Tables(0).Rows.Count
                        Dim fila_a As DataRow = Equipos_ds.Tables("CR_equipo_info").NewRow
                        fila_a("Cliente") = txt_cliente.Text
                        fila_a("Sucursal") = sucursal_text
                        fila_a("Sector") = ds_equipos.Tables(0).Rows(ii).Item("Sector").ToString
                        fila_a("Equipo_descripcion") = ds_equipos.Tables(0).Rows(ii).Item("Equipo_descripcion").ToString
                        fila_a("Equipo_Denominacion") = ds_equipos.Tables(0).Rows(ii).Item("Equipo_denominacion").ToString
                        fila_a("Categoria_tipo") = ds_equipos.Tables(0).Rows(ii).Item("Categoria_tipo").ToString
            ...
            If rb_detalles_no.Checked = True Then
                If Equipos_ds.Tables("CR_equipo_info").Rows.Count <> 0 Then
                    ...
                    Dim visor As New Facturacion_report_show
                    visor.CrystalReportViewer1.ReportSource = CrReport
                    visor.Text = "Reporte de Equipos ordenados por Sector." + sucursal_text + " Imprimir."
                    visor.Show()
                Else
                    'MessageBox.Show("No hay equipos disponibles para listar.", "Sistema de Gestión.", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            ...
            Else
                ...
                If (Equipos_ds.Tables("CR_equipo_info").Rows.Count <> 0) And (Equipos_ds.Tables("CR_equipo_info_caracteristicas").Rows.Count <> 0) Then
                    ...
                    Dim visor As New Facturacion_report_show
                    visor.CrystalReportViewer1.ReportSource = CrReport
                    visor.Text = "Reporte de Equipos ordenados por Sector. Sucursal:" + sucursal_text + ". Imprimir."
                    visor.Show()
                Else
                    'MessageBox.Show("No hay equipos disponibles para listar.", "Sistema de Gestión.", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
    ...
    Private Sub rb_1_todo_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles rb_1_todo.CheckedChanged
        If rb_1_todo.Checked = True Then
            rb_solo_sector.Enabled = False
            cb_sector.Enabled = False
            rb_todos_suc.Checked = True
        Else
            rb_solo_sector.Enabled = True
