...
Public Class Producto_alta_New
    ...
    Public Sub recuperar_datos()
        ...
        tx_codinterno.Text = ds_prod.Tables(0).Rows(0).Item("prod_codinterno")
        tx_descripcion.Text = ds_prod.Tables(0).Rows(0).Item("prod_descripcion")
        tx_descrilarga.Text = ds_prod.Tables(0).Rows(0).Item("prod_descrilarga").ToString
        tx_unimedida.Text = ds_prod.Tables(0).Rows(0).Item("prod_unidadmedida")
        tx_codbarra.Text = ds_prod.Tables(0).Rows(0).Item("prod_codbarra")
        tx_ptorepo.Text = ds_prod.Tables(0).Rows(0).Item("prod_ptorepo")
        tx_stock.Text = ds_prod.Tables(0).Rows(0).Item("prod_stock")
        Tb_PrecMin.Text = ds_prod.Tables(0).Rows(0).Item("prod_precio_vta")
        tb_PrecMay.Text = ds_prod.Tables(0).Rows(0).Item("prod_precio_vta_May").ToString
        cb_marca.SelectedValue = ds_prod.Tables(0).Rows(0).Item("marca_id")
        Combo_tipo.SelectedItem = ds_prod.Tables(0).Rows(0).Item("prod_tipo")

        '///////////choco 11-03-2021 precio_editable
        If ds_prod.Tables(0).Rows(0).Item("prod_precio_editable").ToString = "no" Then
            Rb_precioeditable_no.Checked = True
        Else
            Rb_precioeditable_si.Checked = True
        End If
        '///////////////////////////////////////////
    ...
    Private Sub producto_alta()
        ...
        If valida = "Si" Then
            ...
            If result = DialogResult.OK Then
                ...
                Dim precio_editable As String 'me dice si se puede modificar el precio
                If Rb_precioeditable_no.Checked = True Then
                    precio_editable = "no"
                Else
                    precio_editable = "si"
                End If

                Dim ds_prodid As DataSet = DAproducto.Producto_Alta_New(tx_descripcion.Text, tx_descrilarga.Text, Tb_PrecMin.Text, 0, CInt(tx_ptorepo.Text), tx_codinterno.Text, tx_codbarra.Text, idcat, nrocat, idmarca, tx_unimedida.Text, producto_foto, tb_PrecMay.Text, Combo_tipo.SelectedItem, precio_editable)
    ...
    Private Sub Rutina_Producto_modificar()
        ...
        If valida = "Si" Then
            ...
            If result = DialogResult.OK Then
                ...
                Dim ds_prodid As DataSet = DAproducto.Producto_modificar(tx_descripcion.Text, tx_descrilarga.Text, Tb_PrecMin.Text, tx_codinterno.Text, tx_codbarra.Text, idcat, nrocat, idmarca, tx_unimedida.Text, producto_foto, tb_PrecMay.Text, CInt(tx_ptorepo.Text), Combo_tipo.SelectedItem, precio_editable)
                Dim prodid As Integer = ds_prodid.Tables(0).Rows(0).Item(0)
