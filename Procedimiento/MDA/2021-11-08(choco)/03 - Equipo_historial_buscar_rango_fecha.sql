USE [SantaFe]
GO
/****** Object:  StoredProcedure [dbo].[Equipo_historial_buscar_rango_fecha]    Script Date: 11/08/2021 23:34:08 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER procedure [dbo].[Equipo_historial_buscar_rango_fecha]
@Equipo_id int,
...
inner join Equipo_categoria2 on Equipo.Equipo_idcat = Equipo_categoria2.Cat2_equipo_id 
inner join Equipo_categoria1 on Equipo_categoria2.Cat1_equipo_id = Equipo_categoria1.Cat1_equipo_id 
where Equipo.Equipo_id = @Equipo_id and (Mantenimiento_realizado.Mant_realizados_fecha between @fecha_desde and @fecha_hasta) order by Mantenimiento_realizado.Mant_realizados_fecha  asc
...
inner join Cliente_suc_sector on Equipo.Cliente_suc_sector_id = Cliente_suc_sector.Cliente_suc_sector_id 
inner join Cliente_Sucursales on Cliente_suc_sector.SucxClie_id = Cliente_Sucursales.SucxClie_id 
where Equipo.Equipo_id = @Equipo_id and (Mantenimiento_realizado.Mant_realizados_fecha between @fecha_desde and @fecha_hasta) order by dbo.Mantenimiento_realizado.Mant_realizados_id asc
...
where Servicio.Equipo_id = @Equipo_id and Servicio.Servicio_Estado = 'PENDIENTE' and (Servicio.Servicio_FechaRev between @fecha_desde and @fecha_hasta) order by Servicio.Servicio_FechaRev asc
...
where Servicio.Equipo_id = @Equipo_id  and (Servicio.Servicio_Estado <> 'PENDIENTE') AND (Servicio.Servicio_Estado <> 'ANULADO') and (Servicio.Servicio_FechaRep between @fecha_desde and @fecha_hasta) order by Servicio.Servicio_FechaRep asc
